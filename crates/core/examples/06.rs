...
fn main() -> io::Result<()> {
    ...
    let viewport_top_left = camera_center - Vec3::Z - viewport_u / 2.0 - viewport_v / 2.0;
    ...
    for j in 0..image_height {
        ...
        for i in 0..image_width {
            ...
            let hit_sphere = |center, radius, ray: Ray| {
                // if b = -2h
                ...
                if discriminant < 0.0 { -1.0 } else { (h - discriminant.sqrt()) / a }
            };

            let color = {
                let t = hit_sphere(Point3::NEG_Z, 0.5, ray);
